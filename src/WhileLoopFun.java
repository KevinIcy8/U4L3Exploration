public class WhileLoopFun
{
    ...
    public void printDigits(int number)
    {
        ...
        while(length>0)
        {
            System.out.println(sNumber.substring(length-1, length-2));
            length--;
        }
    }
    ...
    public int sumOfDigits(int number)
    {
        String sNumber = String.valueOf(number);
        int length = sNumber.length();
        int sum = Integer.parseInt(sNumber.substring(length-1, length-2));
        while(length>0)
        {
            sum = sum + Integer.parseInt(sNumber.substring(length-2, length-3));
            length--;
        }
        return sum;
    }
    ...
    public boolean isPrime(int number)
    {
        ...
        while (divisible < 3)
        {
            int ifDivisible = number % divisible;
            if (ifDivisible == 0)
            {
                divisible++;
            }
